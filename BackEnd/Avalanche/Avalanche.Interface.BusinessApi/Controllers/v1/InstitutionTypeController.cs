...
namespace Avalanche.Interface.BusinessApi.Controllers.v1
{
    ...
    public class InstitutionTypeController : BaseApiController
    {
        ...
        [HttpGet()]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(GetAllInstitutionTypeQueryResponse))]
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(ErrorDTO))]
        [ProducesResponseType(StatusCodes.Status401Unauthorized, Type = typeof(ErrorDTO))]
        [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(ErrorDTO))]
        [SwaggerOperation(
           Summary = "Obtener todos los tipos de instituciones",
           Description = "Nos permite obtener todos los tipos de instituciones disponibles en el sistema"
        )]
        public async Task<IActionResult> GetInstitutionTypes()
        {
            try
            {
                var result = await Mediator.Send(new GetAllInstitutionTypeQuery());

                if (result.InstitutionTypes.Count == 0)
                {
                    return NotFound(ErrorMapperHelper.Error(ErrorMessages.NotFound, "No existen tipos de tipos de instituciones en el sistema"));
                }
                return Ok(result);
            }
            catch (Exception e)
